...
func TestCaptureKitCLIOffline(t *testing.T) {
	...
	out, stderr, e := runCLI(t, "--db", db, "research", "evidence", "create", "--output", kit, "--site-code", "ABC", "--management-point", "mp01.lab.local", "--client-label", "win11-client-a", "--capture-label", "baseline-01")
	if e != nil {
	...
	if _, _, e = runCLI(t, "--db", db, "research", "evidence", "validate", "--directory", kit); e != nil {
		t.Fatal(e)
	}
	...
	out, stderr, e = runCLI(t, "--db", db, "research", "capture", "guided-import", "--kit", kit, "--dry-run", "--format", "json")
	if e != nil {
	...
	out, stderr, e = runCLI(t, "--db", db, "research", "capture", "guided-import", "--kit", kit)
	if e != nil {
...
func TestCaptureEvidenceBundleCLIOffline(t *testing.T) {
	...
	if _, _, e := runCLI(t, "--db", db, "research", "evidence", "create", "--output", kit); e != nil {
		t.Fatal(e)
	}
	...
	if _, stderr, e := runCLI(t, "--db", db, "research", "evidence", "bundle", "export", "--directory", kit, "--output", bundle, "--format", "json"); e != nil {
		t.Fatalf("export %v %s", e, stderr)
	}
	out, _, e := runCLI(t, "research", "evidence", "bundle", "inspect", "--input", bundle, "--format", "json")
	...
	if _, _, e = runCLI(t, "--db", db, "research", "evidence", "bundle", "sign", "--input", bundle, "--key", key, "--output", signed); e != nil {
		t.Fatal(e)
	}
	if _, _, e = runCLI(t, "research", "evidence", "bundle", "verify", "--input", signed); e != nil {
		t.Fatal(e)
	}
	if _, _, e = runCLI(t, "--db", db, "research", "capture", "guided-import", "--bundle", signed, "--dry-run"); e != nil {
		t.Fatal(e)
	}
	if _, _, e = runCLI(t, "research", "capture", "guided-import", "--kit", kit, "--bundle", signed); e == nil {

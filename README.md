...
Run-scoped artifacts use a canonical registry rather than domain-specific path handoffs. Public assessment workflows accept target/run context only; research direct-file mode uses common artifact options. Safe profile bounds are automatic, while the `research` profile permits larger offline-only limits without satisfying any safety acknowledgement.

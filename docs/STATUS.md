...
Runtime validation on 2026-08-02 covered all three generated scripts on one disposable GOAD Windows Server 2019 Standard Evaluation build 17763 client using Windows PowerShell Desktop 5.1.17763.8510 and Configuration Manager client 5.00.9128.1007. The first run identified and corrected PowerShell 5.1-incompatible SHA-256 and UTF-8-no-BOM APIs. The corrected inventory, preparation, and finalization scripts exited successfully with empty stderr. Bounded pre/post checks found the SCCM service, client version, site assignment, identity fingerprints, execution policies, firewall profiles, certificate-store fingerprints, task/software counts, and capture-process set unchanged. The account was an administrator and process-scoped bypass was used; privilege necessity and other Windows versions remain unverified. The test used the isolated GOAD inventory's pre-existing HTTPS certificate-validation exception only for the authorized client.

The first passive natural-activity capture then ran for 600 seconds with the existing `pktmon` 10.0.17763.3650 on one active adapter, a 64 MiB circular limit, full packet snapshots, and no traffic filter. It stopped cleanly with 212 packets and zero tool-reported drops; the original ETL was preserved and converted locally to PCAPNG. CinderPath decoded 212 PCAPNG packet records, reconstructed zero supported flows and zero HTTP exchanges, and emitted opaque-TLS plus incomplete/conflicting TCP-reassembly warnings. This is metadata-only endpoint evidence, not positive policy-exchange evidence. No policy action was triggered, the bounded pre/post state remained unchanged, and raw artifacts were not committed. Runtime evidence also exposed and fixed a state-evaluation defect: generated preparation/finalization marker files now drive `capture_in_progress` and `requires_sanitization` even when operator metadata timestamps remain blank.

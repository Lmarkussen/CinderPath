...
Misconfiguration Manager coverage is sourced from the embedded offline snapshot in `internal/framework/data`. Use independent support dimensions and keep upstream import/validation under `research framework`; normal runtime must never download the upstream catalog.

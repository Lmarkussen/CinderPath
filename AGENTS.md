...
The primary operator workflow remains mock-safe. Policy research is offline and fixture-driven; schema-v3 persistence and ordinary reports must contain redacted metadata only. Live policy execution remains blocked unless a separately reviewed contract reaches `approved_live`, and normal commands may not promote contracts. Preserve loopback-only replay, bounded binary/XML analysis, client-metadata-only import, deliberate plaintext output, and the Windows lab capture safeguards. The next task is expanding reviewed binary fixtures, sanitizer modes, and signed research bundles without adding registration, content requests, protected-secret decryption, or state changes.

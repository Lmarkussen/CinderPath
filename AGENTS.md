...
The primary operator workflow remains mock-safe. Policy research is offline and fixture-driven; schema-v3 persistence and ordinary reports contain redacted metadata only. Durable module decisions, standalone dry-run histories, binary-aware sanitizer modes, manual review, fingerprinted unsigned bundles, loopback fixture lifecycle, capture plans, and policy inventories are implemented. Live policy execution remains blocked unless a separately reviewed contract reaches `approved_live`, and normal commands may not promote contracts. Preserve loopback-only replay, bounded binary/XML analysis, client-metadata-only import, deliberate plaintext output, and the Windows lab capture safeguards. The next task is collecting and comparing reviewed authorized-lab fixtures, improving supported structured parsers, and designing signed research bundles without registration, content requests, protected-secret decryption, or state changes.
